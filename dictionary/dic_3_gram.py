...
output_file = "dic_3_gram_test.txt"
    ...
    for words_sent in words_sent_tokenize:
        ...
        if len(tokens) > 2:
            for i in range(1,len(tokens)-1):
                word_full = tokens[i-1] + "_" + tokens[i] + "_" + tokens[i + 1]
        ...
        if len(token_splits) > 1:
            for i in range(1, len(token_splits)-1):
                word_split = token_splits[i-1] + "_" + token_splits[i] + "_" + token_splits[i + 1]
